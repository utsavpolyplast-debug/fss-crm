...
import { supabase, Party, AreaPlan } from '@/lib/supabase'
...
export default function FieldHome() {
  ...
  const [todayPlan, setTodayPlan] = useState<AreaPlan | null>(null)
    ...
    async function load() {
      ...
      if (plan) {
        ...
        const { data: visits } = await supabase
          ...
          .eq('user_id', session.user.id)
          .eq('visit_date', today)
        setVisitedIds(new Set((visits || []).map((v: any) => v.party_id)))
      } else {
        // No plan assigned — show all assigned parties in their area
        const { data: pts } = await supabase
          .from('parties')
          .select('*')
          .eq('assigned_to', session.user.id)
          .in('status', ['Active', 'Follow-up'])
          .order('priority', { ascending: true })
          .limit(15)
        setParties(pts || [])
      }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-brand-500 text-white px-4 pt-10 pb-6">
        <div className="flex items-center justify-between mb-1">
          ...
          <button onClick={logout} className="text-blue-200 text-xs border border-blue-300 px-2 py-1 rounded-lg">
            Logout
          </button>
        </div>
        {todayPlan && (
        ...
        <div className="px-4 -mt-1">
          <div className="bg-blue-100 rounded-full h-1.5">
            <div className="bg-green-400 h-1.5 rounded-full transition-all"
              style={{ width: `${Math.round((done.length / parties.length) * 100)}%` }} />
          </div>
      ...
      <div className="px-4 py-4 space-y-4">
            ...
            <div className="space-y-2">
              {pending.map((party, i) => (
                <div key={party.id} className="card flex items-center gap-3"
                  onClick={() => router.push(`/field/checkin?party=${party.id}`)}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${i === 0 ? 'bg-brand-500 text-white' : 'bg-gray-100 text-gray-500'}`}>
                    {i + 1}
                  </div>
            ...
            <div className="space-y-2">
              {done.map(party => (
                <div key={party.id} className="card flex items-center gap-3 opacity-70">
                  ...
                  <span className="text-xs text-green-600 font-medium">Visit done</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button onClick={() => router.push('/field/visit')}
            className="card flex flex-col items-center gap-2 py-4 active:scale-95 transition-all">
