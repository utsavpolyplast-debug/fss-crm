...
export default function FieldSchedule() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: rawPlans } = await supabase.from('area_plans').select('*').eq('user_id', session.user.id).in('plan_date', weekDates)
          ...
          {weekDays.map((day, i) => {
            ...
            return (
              <button key={i} onClick={() => setSelectedDay(i)}
                className={'flex flex-col items-center px-3 py-2 rounded-xl transition-all ' + (selectedDay === i ? 'bg-brand-500 text-white' : isToday ? 'bg-blue-50 border border-brand-500 text-brand-500' : 'bg-white border border-gray-200 text-gray-600')}>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {selectedDate === todayStr && (
                <button className="bg-brand-500 text-white text-xs px-3 py-1.5 rounded-lg" onClick={() => router.push('/field/home')}>Start today's visits</button>
              )}
            </div>
            {selectedPlan.notes && <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 text-sm text-amber-800">{selectedPlan.notes}</div>}
            <div className="space-y-2">
              {selectedPlan.parties.map((party, i) => (
                <div key={party.id} className="card flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-blue-50 flex items-center justify-center text-xs font-bold text-brand-500">{i + 1}</div>
