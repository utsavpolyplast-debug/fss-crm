...
export default function CheckIn() {
  ...
  async function captureGPS() {
    ...
    try {
      ...
      if (party?.lat && party?.lng) {
        ...
        setGpsVerified(true) // No party coords stored yet
      }
      setStep('photo')
    } catch (e: any) {
  ...
  async function submitVisit() {
    ...
    // If order placed, save order too
      ...
      <div className="px-4 pb-8 space-y-4">
        ...
        {step === 'gps' && (
          <div className="card space-y-4">
            ...
            <button className="btn-primary" onClick={captureGPS} disabled={gpsLoading}>
              {gpsLoading ? (
                <><span className="animate-spin">⟳</span> GPS dhundh raha hai...</>
              ) : (
                <><span>📍</span> Meri location capture karo</>
              )}
            </button>
          </div>
        )}
        ...
        {step === 'photo' && (
          <div className="card space-y-4">
                  ...
                  <div>
                    ...
                    {gpsDistance > 0 && (
                      <p className="text-xs text-gray-500">
                        Party se {formatDistance(gpsDistance)} door ho
                      </p>
                    )}
                  </div>
                </div>
            ...
            <button className="btn-primary" onClick={() => fileRef.current?.click()}>
              <span>📸</span> Camera open karo
            </button>
            <button className="btn-secondary text-xs" onClick={() => setStep('form')}>
              Skip photo (urgent ho toh)
            </button>
          </div>
        )}
            ...
            <div className="card space-y-4">
              ...
              <div>
                ...
                <input className="input" value={items} onChange={e => setItems(e.target.value)}
                  placeholder="e.g. Oval 500ml, Sipper XL" />
              </div>
              ...
              <div>
                ...
                <textarea className="input" rows={3} value={remarks} onChange={e => setRemarks(e.target.value)}
                  placeholder="Kya hua visit mein..." />
              </div>
        ...
        {step === 'done' && (
          <div className="card text-center py-10 space-y-4">
            ...
            <button className="btn-primary" onClick={() => router.replace('/field/home')}>
              Ghar wapas jao
            </button>
          </div>
        )}
