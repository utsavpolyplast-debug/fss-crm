...
function CheckInInner() {
  ...
  useEffect(() => {
    ...
    supabase.from('parties').select('*').eq('id', partyId).single().then(({ data }) => { if (data) setParty(data) })
  }, [partyId, router])
  ...
  async function submitVisit() {
    ...
    const visitData = {
      party_id: party.id, user_id: session.user.id,
      visit_date: format(new Date(), 'yyyy-MM-dd'),
      checkin_time: new Date().toISOString(),
      gps_lat: gpsLat, gps_lng: gpsLng,
      gps_verified: gpsVerified,
      gps_distance_meters: Math.round(gpsDistance),
      photo_url, outcome, items_discussed: items, remarks,
    ...
    if (outcome === 'Order Placed' && items && qty) {
      await supabase.from('orders').insert({
        party_id: party.id, visit_id: visit ? visit.id : null,
        user_id: session.user.id,
        order_date: format(new Date(), 'yyyy-MM-dd'),
        item: items, qty_cartons: parseInt(qty) || 0,
        rate_per_carton: parseFloat(rate) || 0,
        total_amount: (parseInt(qty) || 0) * (parseFloat(rate) || 0),
        payment_status: 'Pending', remarks
      })
    }
    if (outcome === 'Order Placed') await supabase.from('parties').update({ status: 'Active' }).eq('id', party.id)
    else if (outcome === 'Follow-up') await supabase.from('parties').update({ status: 'Follow-up' }).eq('id', party.id)
    setStep('done')
    setSaving(false)
  }
      ...
      <div className="px-4 pb-8 space-y-4 mt-4">
        {step === 'gps' && (
          <div className="card space-y-4">
            ...
            <button className="btn-primary" onClick={captureGPS} disabled={gpsLoading}>{gpsLoading ? 'GPS dhundh raha hai...' : 'Meri location capture karo'}</button>
          </div>
        )}
            ...
            <div className="card space-y-4">
              ...
              <div>
                ...
                <select className="input" value={outcome} onChange={e => setOutcome(e.target.value)}>
                  <option>Meeting Done</option><option>Order Placed</option><option>Follow-up</option><option>Sample Given</option><option>Not Interested</option>
                </select>
              </div>
