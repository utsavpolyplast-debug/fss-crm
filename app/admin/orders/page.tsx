      ...
      <div className="px-4 py-3 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="card text-center">
            ...
            <p className="text-xl font-bold text-red-600">₹{pending.toLocaleString('en-IN')}</p>
          </div>
        </div>
        ...
        {loading ? <div className="text-center py-8 text-gray-400">Loading...</div> : (
          <div className="space-y-2">
            {orders.map(o => (
              <div key={o.id} className="card">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-semibold text-sm text-gray-900">{(o.parties as any)?.name}</p>
                    <p className="text-xs text-gray-400">{o.order_date} · {(o.parties as any)?.area}</p>
                    <p className="text-xs text-gray-600 mt-1">{o.item} · {o.qty_cartons} cartons × ₹{o.rate_per_carton}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-gray-900">₹{(o.total_amount || 0).toLocaleString('en-IN')}</p>
            ...
            {orders.length === 0 && (
              <div className="card text-center py-8 text-gray-400 text-sm">Koi order nahi mila</div>
            )}
          </div>
        )}
      </div>
