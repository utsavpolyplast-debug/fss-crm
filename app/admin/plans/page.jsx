...
export default function AdminPlans() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setFieldBoys(boys || [])
      if (boys && boys.length) setSelUser(boys[0].id)
  ...
  const areaParties = parties.filter(p => p.area === selArea)
  ...
  async function savePlan() {
    ...
    if (existing) {
      await supabase.from('area_plans').update({ area: selArea, party_ids: selParties, notes, created_by: session.user.id }).eq('id', existing.id)
    } else {
      await supabase.from('area_plans').insert({ user_id: selUser, plan_date: selDate, area: selArea, party_ids: selParties, notes, created_by: session.user.id })
    }
    const weekDates = weekDays.map(d => format(d, 'yyyy-MM-dd'))
    const { data: ps } = await supabase.from('area_plans').select('*, profiles(name)').in('plan_date', weekDates)
    setPlans(ps || [])
      ...
      <div className="px-4 pt-4 pb-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs bg-white rounded-xl border border-gray-100 overflow-hidden">
            <thead><tr className="bg-gray-50"><th className="text-left px-3 py-2 text-gray-500">Din</th><th className="text-left px-3 py-2 text-gray-500">Field Boy</th><th className="text-left px-3 py-2 text-gray-500">Area</th><th className="text-left px-3 py-2 text-gray-500">Parties</th></tr></thead>
            <tbody>
              {weekDays.map((day, i) => {
                ...
                return (
                  <tr key={i} className="border-t border-gray-50">
                    ...
                    <td className="px-3 py-2 text-gray-500">{dayPlans.map(p => p.profiles && p.profiles.name).join(', ') || '-'}</td>
                    <td className="px-3 py-2 text-gray-500">{dayPlans.map(p => p.area).join(', ') || '-'}</td>
                    <td className="px-3 py-2 text-gray-500">{dayPlans.reduce((a, p) => a + p.party_ids.length, 0) || 0}</td>
                  </tr>
                )
              })}
            </tbody>
      ...
      <div className="px-4 py-3 space-y-4">
        ...
        <div className="card space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Field Boy</label><select className="input" value={selUser} onChange={e => setSelUser(e.target.value)}>{fieldBoys.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {DELHI_AREAS.map(area => (
                <button key={area} onClick={() => selectArea(area)} className={'px-3 py-1.5 rounded-full text-xs font-medium transition-all ' + (selArea === area ? 'bg-brand-500 text-white' : 'bg-gray-100 text-gray-600')}>
                  {area}
                </button>
              ))}
            </div>
          </div>
          <div><label className="label">Notes</label><input className="input" value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Pehle Laxmi Nagar side jaana" /></div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              <button className="text-xs text-brand-500" onClick={() => setSelParties(areaParties.map(p => p.id))}>Sab select</button>
              <button className="text-xs text-gray-400" onClick={() => setSelParties([])}>Clear</button>
            </div>
          </div>
          {areaParties.length === 0 && <div className="card text-center py-6 text-gray-400 text-sm">{selArea} mein koi party nahi</div>}
          <div className="space-y-2">
            {areaParties.map(party => (
              <div key={party.id} className={'card flex items-center gap-3 cursor-pointer ' + (selParties.includes(party.id) ? 'border-brand-500 bg-blue-50' : '')} onClick={() => toggleParty(party.id)}>
                <div className={'w-5 h-5 rounded flex items-center justify-center flex-shrink-0 border-2 ' + (selParties.includes(party.id) ? 'bg-brand-500 border-brand-500' : 'border-gray-300')}>
                  {selParties.includes(party.id) && <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg>}
                </div>
        ...
        {saved && <div className="bg-green-50 border border-green-200 text-green-700 text-sm rounded-xl px-4 py-3 text-center font-medium">Plan save ho gaya! Field boy ko dikhai dega</div>}
