...
import { supabase, Party, Profile } from '@/lib/supabase'
import { badgeColor, DELHI_AREAS, PARTY_TYPES, PARTY_STATUSES } from '@/lib/utils'
...
export default function AdminParties() {
  ...
  const [form, setForm] = useState({
    name: '', contact_person: '', mobile: '', city: '', area: 'East Delhi',
    address: '', lat: '', lng: '', type: 'Existing Buyer', status: 'Active',
    products_interested: '', priority: '5', assigned_to: '', notes: ''
  })
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-brand-500 text-white px-4 pt-10 pb-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-200 text-xs">{parties.length} total parties</p>
          </div>
          <button onClick={() => setShowAdd(true)}
            className="bg-white text-brand-500 text-sm font-medium px-3 py-1.5 rounded-xl">
            + Add Party
          </button>
        </div>
      </div>
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        <div className="space-y-2">
          <input className="input" placeholder="Search party name, city..." value={search}
            onChange={e => setSearch(e.target.value)} />
          <div className="flex gap-2 overflow-x-auto pb-1">
            <button onClick={() => setAreaFilter('')}
              className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 ${!areaFilter ? 'bg-brand-500 text-white' : 'bg-white border border-gray-200 text-gray-600'}`}>
              All Areas
            </button>
            {DELHI_AREAS.map(a => (
              <button key={a} onClick={() => setAreaFilter(a === areaFilter ? '' : a)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 ${areaFilter === a ? 'bg-brand-500 text-white' : 'bg-white border border-gray-200 text-gray-600'}`}>
                {a}
              </button>
            ))}
          </div>
          <div className="flex gap-2">
            {['', ...PARTY_TYPES].map(t => (
              <button key={t} onClick={() => setTypeFilter(t)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 ${typeFilter === t ? 'bg-gray-700 text-white' : 'bg-white border border-gray-200 text-gray-600'}`}>
                {t || 'All Types'}
              </button>
            ))}
          </div>
          ...
          {filtered.map(p => (
            <div key={p.id} className="card">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-sm font-bold text-brand-500 flex-shrink-0">
                  {p.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-2 mt-1 flex-wrap">
                    ...
                    {!p.lat && <span className="badge bg-orange-100 text-orange-700">No GPS coords</span>}
                  </div>
                </div>
              </div>
            </div>
        ...
        {showAdd && (
          <div className="card space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Contact Person</label><input className="input" value={form.contact_person} onChange={e => setForm(f=>({...f,contact_person:e.target.value}))} placeholder="Ramesh Ji"/></div>
              <div><label className="label">Mobile</label><input className="input" type="tel" value={form.mobile} onChange={e => setForm(f=>({...f,mobile:e.target.value}))} placeholder="9811234567"/></div>
            </div>
            <div>
            ...
            <div><label className="label">Address (GPS verify ke liye)</label><input className="input" value={form.address} onChange={e => setForm(f=>({...f,address:e.target.value}))} placeholder="Shop no, street, colony"/></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Latitude (Google Maps se)</label><input className="input" type="number" step="0.0001" value={form.lat} onChange={e => setForm(f=>({...f,lat:e.target.value}))} placeholder="28.6562"/></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select className="input" value={form.status} onChange={e => setForm(f=>({...f,status:e.target.value}))}>
                  {PARTY_STATUSES.map(s => <option key={s}>{s}</option>)}
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select className="input" value={form.assigned_to} onChange={e => setForm(f=>({...f,assigned_to:e.target.value}))}>
                  <option value="">Any field boy</option>
                  {fieldBoys.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                </select>
              </div>
              <div>
                <label className="label">Priority (1=high)</label>
                <input className="input" type="number" min="1" max="10" value={form.priority} onChange={e => setForm(f=>({...f,priority:e.target.value}))}/>
              </div>
            </div>
            <div><label className="label">Notes</label><textarea className="input" rows={2} value={form.notes} onChange={e => setForm(f=>({...f,notes:e.target.value}))} placeholder="Koi khaas baat..."/></div>
            <div className="flex gap-2">
