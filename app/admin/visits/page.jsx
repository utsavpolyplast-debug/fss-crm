...
export default function AdminVisits() {
  ...
  useEffect(() => {
    async function load() {
      ...
      let q = supabase.from('visits').select('*, parties(name,city,area), profiles(name)').order('checkin_time', { ascending: false }).limit(100)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-3 space-y-3">
        <div className="card space-y-3">
          <div><label className="label">Date</label><input className="input" type="date" value={dateFilter} onChange={e => setDateFilter(e.target.value)} /></div>
          <div>
            <label className="label">GPS status</label>
            <div className="flex gap-2">
              {[['','All'],['verified','Verified only'],['mismatch','Mismatch only']].map(([val, label]) => (
                <button key={val} onClick={() => setGpsFilter(val)} className={'px-3 py-1.5 rounded-full text-xs font-medium ' + (gpsFilter === val ? 'bg-brand-500 text-white' : 'bg-gray-100 text-gray-600')}>
                  {label}
                </button>
              ))}
            </div>
              ...
              <div key={v.id} className={'card ' + (!v.gps_verified ? 'border-red-200' : '')}>
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{v.profiles && v.profiles.name} · {format(new Date(v.checkin_time), 'hh:mm a')} · {v.parties && v.parties.area}</p>
                    <div className="flex gap-2 mt-1 flex-wrap">
                      <span className={'text-xs px-2 py-0.5 rounded-full font-medium ' + (v.gps_verified ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700')}>
                        {v.gps_verified ? 'GPS ok' : 'GPS far ' + (v.gps_distance_meters > 0 ? '(' + formatDistance(v.gps_distance_meters) + ')' : '')}
                      </span>
                      {v.photo_url
                  ...
                  {v.photo_url && <img src={v.photo_url} className="w-16 h-16 rounded-xl object-cover flex-shrink-0 cursor-pointer" onClick={() => setBigPhoto(v.photo_url)} alt="proof" />}
                </div>
              </div>
            ))}
            {visits.length === 0 && <div className="card text-center py-8 text-gray-400 text-sm">Koi visit nahi mili</div>}
          </div>
        )}
      </div>

      {bigPhoto && (
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 p-4" onClick={() => setBigPhoto('')}>
          <img src={bigPhoto} className="max-w-full max-h-full rounded-xl" alt="Visit proof" />
          <button className="absolute top-4 right-4 text-white text-2xl font-bold">x</button>
        </div>
      )}
    </div>
  )
